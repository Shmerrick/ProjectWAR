...
$output = Join-Path $BuildRoot 'SkavenFormChecks.exe'
& $compiler /nologo /warnaserror+ /target:exe /platform:x64 "/out:$output" `
    ...
    (Join-Path $PSScriptRoot 'SkavenFormChecks.cs')
if ($LASTEXITCODE -ne 0) { throw 'Skaven form check compilation failed.' }
& $output
if ($LASTEXITCODE -ne 0) { throw 'Skaven form data checks failed.' }
