...
$output = Join-Path $BuildRoot 'AbilityAlignmentChecks.exe'
& $compiler /nologo /warnaserror+ /target:exe /platform:x64 "/out:$output" `
    ...
    (Join-Path $PSScriptRoot 'AbilityAlignmentChecks.cs')
if ($LASTEXITCODE -ne 0) { throw 'Ability alignment check compilation failed.' }
& $output
if ($LASTEXITCODE -ne 0) { throw 'Ability alignment checks failed.' }
