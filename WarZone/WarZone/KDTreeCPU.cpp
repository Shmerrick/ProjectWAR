...
KDTreeCPU::KDTreeCPU( int num_tris, glm::vec3 *tris, int num_verts, glm::vec3 *verts, int *triangleIDs,  int NUM_TRIS_PER_NODE )
{
	// Set class-level variables.
	num_levels = 0;
	num_leaves = 0;
	num_nodes = 0;
	this->NUM_TRIS_PER_NODE = NUM_TRIS_PER_NODE;
	...
	this->triangleIDs = new int[num_tris];

	this->visible = new bool[num_tris];
	for ( int i = 0; i < num_tris; ++i ) {
		this->tris[i] = tris[i];
		this->visible[i] = true;
		this->triangleIDs[i] = triangleIDs[i];
	}
...
int KDTreeCPU::intersect( const glm::vec3 &ray_o, const glm::vec3 &ray_dir, float &t, glm::vec3 &hit_point, glm::vec3 &normal ) const
{
	t = INFINITY;
	int tagA = 0;
	int tagB = 0;
	int hit = intersect( root, ray_o, ray_dir, t, normal );
	if ( hit ) {
		hit_point = ray_o + ( t * ray_dir );
	}
	return hit;
}
void KDTreeCPU::SetTriangleVisible(int index, bool visible)
{
	this->visible[index] = visible;
}
// Private recursive call.
int KDTreeCPU::intersect( KDTreeNode *curr_node, const glm::vec3 &ray_o, const glm::vec3 &ray_dir, float &t, glm::vec3 &normal ) const
{
	...
	if ( intersects_aabb ) {
		...
		if ( !curr_node->left && !curr_node->right ) {
			...
			{
				int ind = curr_node->tri_indices[i];
				if(visible[curr_node->tri_indices[i]]){
					...
					if ( intersects_tri ) 
					{
						intersection_detected = triangleIDs[curr_node->tri_indices[i]];
						if ( tmp_t < t ) 
						{
							hit = intersection_detected;
							t = tmp_t;
							normal = tmp_normal;
						}
					}
