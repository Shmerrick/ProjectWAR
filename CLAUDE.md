...
Current evidence review and measured readiness: [September 24 audit](docs/handoffs/2026-09-24-repository-audit.md).
It corrects stale Skaven component-absence claims and separates database checks from
gameplay acceptance. Earlier checkpoint numbers are historical, not current measurements.
...
This machine's dynamic TCP range is **1024–65534** (Windows default is 49152+), so any outbound connection can occupy a fixed emulator port. This has bitten three times: `msedgewebview2` took 10300, Unreal Engine's MCP server took 8000, and on 2026-09-14 the `\MortalAtlasHost` scheduled task (Bun serving the Mortal_RE Atlas site, started at boot) held `127.0.0.1:8000`, so the launcher, which hard-codes `127.0.0.1:8000`, could not log in. A port exclusion stops ephemeral grabs only; a program that binds the port by number still gets it.
...
Gameplay is served from in-memory caches — after boot, essentially the only recurring query is the character load. That makes the per-login lookups the scaling path, and four of them were unindexed full table scans until indexes were added to them (now part of the `war_characters` base dump). If you add a per-character or per-account query, check `EXPLAIN` shows `type: ref` rather than `type: ALL`.
...
1. **Never edit the base dumps** `Database/war_accounts.sql`, `war_characters.sql`, `war_world.sql`. Schema/data changes ship as a new numbered incremental script in `Database/`, taking the next free number and delivered with the code change. The previous series (01-98) has been folded into the base dumps and deleted, so the numbering restarted at `00`: take the number after the highest script currently in `Database/`. `AGENTS.md` rule 6 goes further than "the user applies it": apply the script to the local Release build's database and verify the resulting schema and data — never report database-backed work as tested when only the compile was checked. Note that the ORM auto-provisions unknown tables — `ObjectDatabase` calls `CheckOrCreateTable`, which issues `CREATE TABLE IF NOT EXISTS` on registration — so a new `DataObject` entity does not strictly need a migration script to work; write one only when existing rows need changing.
   ...
   **The same flag splits the ability tables, and there the two halves are *not* identical.** `UseMythicActionCoverageTables` also switches `AbilityMgr.LoadNewAbilityInfo` between `abilities`/`buff_infos`/... and `mythic_src_abilities`/`mythic_src_buff_infos`/... (`AbilityMgr.cs:112`). `mythic_src_abilities` is a superset — 8,416 rows against 4,221 — and it had been bulk-populated from `data/gamedata/abilities.csv` (imported as `mythic_csv_abilities`) joined on that art-authoring sheet’s own ID column, which agrees with the client’s real ability ids on 13 of 3,115 because it holds effect ids (the `EffectId` of each `abilityexport.bin` record) — so its `Name`, `EffectID` and `IconId` belonged to other abilities entirely, and `EffectID` goes straight out in the cast packets. Migrations 76 and 77 repaired it. Same rule as items: **write both, or say why one is correct** — and check any ability row against `mythic_bin_ability` (the client) before trusting either. See `docs/ABILITY_TABLE_ALIGNMENT.md`.

   **The client wins on balance values too** (the repository owner's rule, 2026-09-14: conform to the client unless physically unable). Migrations 03-09 took its cooldowns, cast times, AP costs, ranges, EffectIDs, damage and buff durations into both halves. What the client does not hold lives in server-only columns: `AIRange`, and `AICooldown`, how often creature and pet AI casts (the AI waits for the larger of it and `Cooldown`). **Who a cast lands on comes from the client's `TargetType` column** (`AbilityInfo.TargetsCaster`: 0 and 3 mean the caster), not from `Range == 0`; the live captures settle it, and a range of 0 on a targeted ability sets no limit. Channels run on `ChannelDuration` and tick at the client's `ChannelInterval`, and `CastTime` is the client's own -- 0, or the cast before the channel (migration 10). What is still unconformed is a queue with reasons in `docs/DATABASE_FIDELITY_PLAN.md`, not a list of open decisions.
...
3. **Two out-of-repo sources of truth outrank guessing, and are meant to be used actively — not only when stuck.** This is a restoration project: the client is the authoritative record of what the real 1.4.8 server did, and reasoning from this emulator's source alone just reproduces whatever it already got wrong.
   ...
   **Check the client before the database, and make it cheap enough that you always do.** The world
   ...
   `data/gamedata/abilities.csv`, whose ID column holds effect ids and so agrees with the client's real ability ids on 13 of
   ...
   **Abilities have the same witness.** `tools/captures/extract_use_ability.awk` decodes the live
   server's `F_USE_ABILITY` (0xDA) cast-start frames, which carry the ability's EffectID and cast
   time. Against `data/bin/abilityexport.bin` the live EffectID agrees on 1,435 of 1,435 captured
   abilities and the cast time on 1,298 of 1,312, so where the client and a capture agree with each
   other and not with our tables, that is as settled as ability data gets (migrations 01 and 02).
   Take values from kind-1 frames only, and read channels carefully: a channel's kind-1 frame carries 0
   and its length arrives in the kind-3 channel-start frame, which is what migration 10 checked `ChannelDuration` against.
   `tools/captures/extract_use_ability_targets.awk` recovers who each cast landed on, which is how migration 09
   established that the client's TargetType, not the range, picks the target.
...
- `docs/LONDO_DATA_AUDIT.md` — what Londo's 68-table dump contains, that we have imported exactly one of them, and the BonusType stat enumeration in Mythic's own names.
- `docs/ABILITY_DAMAGE_MODEL.md` — how the client computes ability damage (COM tokens, multipliers, ordered components), how the server scales it, and the measured divergences. Read before touching `mythic_src_ability_damage_heals` or anything in `AbilityMgr` that mentions scaling.
- `docs/DATABASE_FIDELITY_PLAN.md` — the measured gap ledger between our database and the client, with the evidence tiers that gate any change.
...
- `docs/handoffs/` — dated session checkpoints. **`2026-09-15-ability-conformance.md` is current** (ability data conformed to the client, migrations 00-10, untested in game, with the conformance queue and the port 8000 problem); before it, **`2026-09-05-gunbad-and-lotd.md`** (Mount Gunbad levels, Gunbad PQ spawn repair, Land of the Dead tracker UI and flight access); `2026-09-05-commit-handoff.md` precedes it; it records the Chaos Wastes entrance regression (BUG-062, diagnosed and fixed in its follow-up pass), PQ retest improvement, retracted blue-orb explanation and remaining work. The earlier checkpoint is historical; use the stabilization handoff for corrected influence-key evidence.
