...
namespace WorldServer.World.AI.PathFinding.AStar
{
    ...
    {
        public static int DetermineH(HeuristicFormula heuristicFormula, Point end, int heuristicEstimate,
            int newLocationY, int newLocationX)
        {
            int h;
            ...
            switch (heuristicFormula)
            {
                ...
                case HeuristicFormula.Euclidean:
                    h = (int) (heuristicEstimate *
                               Math.Sqrt(Math.Pow(newLocationY - end.X, 2) + Math.Pow(newLocationY - end.Y, 2)));

                    break;

                case HeuristicFormula.EuclideanNoSQR:
                    h = (int) (heuristicEstimate *
                               (Math.Pow(newLocationX - end.X, 2) + Math.Pow(newLocationY - end.Y, 2)));
